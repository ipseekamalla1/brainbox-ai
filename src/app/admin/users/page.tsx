...
interface UserItem {
  ...
  createdAt: string;
  _count: { quizAttempts: number; examAttempts: number; notes: number; quizzesCreated: number; examsCreated: number };
}
...
export default function AdminUsersPage() {
  ...
  return (
    <div>
      ...
      {/* Create Form */}
      {showCreate && (
        <form onSubmit={handleCreate} className="mb-6 p-5 rounded-2xl border border-border bg-card space-y-4">
          ...
          <button type="submit" disabled={saving}
            className="px-5 py-2 bg-primary text-primary-foreground text-sm font-semibold rounded-lg hover:opacity-90 disabled:opacity-50">
          ...
          {["", "STUDENT", "TEACHER", "ADMIN"].map((r) => (
            <button key={r} onClick={() => { setRoleFilter(r); setPage(1); }}
              className={`px-3 py-2 rounded-lg text-xs font-semibold border transition-colors ${
                roleFilter === r ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"
              }`}>
              {r || "All"}
            </button>
          ))}
      ...
      {/* Users Table */}
      {loading ? (
        <div className="space-y-3">{[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-16 rounded-xl bg-muted animate-shimmer" />)}</div>
      ) : (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          ...
          {/* Rows */}
          {users.map((user) => (
            <div key={user.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-4 px-4 py-3 border-b border-border last:border-0 items-center hover:bg-muted/20 transition-colors">
