// ============================================================
// FILE LOCATION: app/api/admin/courses/[courseId]/enrollments/route.ts
// ACTION: CREATE NEW
// PURPOSE: GET + POST + DELETE manage who is enrolled in a course
// ============================================================
...
export async function POST(
  ...
  if (parsed.data.action === "assign_teacher") {
    ...
    // Remove existing teacher first
    await prisma.enrollment.updateMany({
      where: { courseId: params.courseId, role: Role.TEACHER, status: "ACTIVE" },
      data: { status: "DROPPED" },
    });
    const result = await assignTeacher(parsed.data.userId, params.courseId);
    return NextResponse.json({ result });
  }

  if (parsed.data.action === "enroll_student") {
    const result = await enrollStudent(parsed.data.userId, params.courseId);
    return NextResponse.json({ result });
  }
  ...
  if (parsed.data.action === "enroll_students_bulk") {
    ...
    for (const userId of parsed.data.userIds) {
      const r = await enrollStudent(userId, params.courseId);
...
export async function DELETE(
  ...
  await prisma.enrollment.updateMany({
    where: { userId, courseId: params.courseId, status: "ACTIVE", ...(role ? { role } : {}) },
    data: { status: "DROPPED" },
  });
